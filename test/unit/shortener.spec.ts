...
describe('Shortener', () => {
    describe('shortNumber()', () => {
        it('should not process an invalid input', () => {
            expect(shortNumber(undefined)).toBe(undefined);
            expect(shortNumber(null)).toBe(null);
            expect(shortNumber('invalid number')).toBe('invalid number');
        });

        it('should convert long number to the short ones with a unit symbol', () => {
            expect(shortNumber(1e+25)).toBe('10Y');
            expect(shortNumber(2.5e+23)).toBe('225Z');
            ...
            expect(shortNumber(100e+6)).toBe('100M');
